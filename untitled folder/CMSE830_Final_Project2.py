...
lst = ['Rating', 'Founded', 'Age']
y = df['Revenue']
y_unique = y.drop_duplicates()
sel1 = st.selectbox('Features', sorted(lst))
sel2 = st.selectbox('Features2', y_unique)
if sel1 and sel2:
    color_mapping = {
        value: color
        for value, color in zip(df[sel1].unique(), px.colors.qualitative.Set1)
    }

    filtered_df = df[df['Revenue'] == sel2]  # Filter DataFrame based on sel2 (assuming 'Revenue' is the column name)

    fig = go.Figure(data=go.Scatter(x=filtered_df[sel1], y=filtered_df[sel2], mode='markers', marker=dict(color=filtered_df[sel1])))
    fig = go.Figure(data=go.Scatter(x=df[sel1], y=filtered_df[sel2], mode='markers', marker=dict(color=df[sel])))
...
with tab3:
    ...
    with tab4:
        view = st.selectbox('view', df_cols)        
        if view:
            fig, ax = plt.subplots(figsize=(8, 6))
            fig = px.box(df, x=df_cols, y= df_cols, color=df_cols, color_discrete_sequence= px.colors.qualitative.Dark24)
